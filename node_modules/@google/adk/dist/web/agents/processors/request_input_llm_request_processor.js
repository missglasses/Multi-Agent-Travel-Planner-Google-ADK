...
const _RequestInputLlmRequestProcessor = class _RequestInputLlmRequestProcessor extends BaseLlmRequestProcessor {
  runAsync(invocationContext) {
    return __asyncGenerator(this, null, function* () {
      ...
      try {
        for (var iter = __forAwait(eventQueue), more, temp, error; more = !(temp = yield new __await(iter.next())).done; more = false) {
          const queuedEvent = temp.value;
          yield queuedEvent;
        }
      } catch (temp) {
        error = [temp];
      ...
      const functionResponseEvent = yield new __await(task);
