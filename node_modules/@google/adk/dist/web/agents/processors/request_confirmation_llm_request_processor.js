...
const _RequestConfirmationLlmRequestProcessor = class _RequestConfirmationLlmRequestProcessor extends BaseLlmRequestProcessor {
  /**
  ...
  runAsync(invocationContext) {
    return __asyncGenerator(this, null, function* () {
      ...
      const resumable = yield new __await(bindApprovedCalls({
        candidates,
        events,
        toolsDict,
...
async function bindApprovedCalls(options) {
  ...
  for (const { pinned, confirmation } of candidates) {
    ...
    const refuse = /* @__PURE__ */ __name((reason) => new IntentMismatchError({ reason, functionCallId: pinnedId }), "refuse");
    ...
    if (!isEqual((_a = original.args) != null ? _a : {}, (_b = pinned.args) != null ? _b : {})) {
...
function agentAuthoredCalls(events, agentName) {
  ...
  for (const event of events) {
    ...
    for (const functionCall of getFunctionCalls(event)) {
      if (functionCall.id && functionCall.name !== REQUEST_CONFIRMATION_FUNCTION_CALL_NAME) {
...
function hasRespondedAfter(events, pinnedCallId, gateIndex) {
  return events.slice(gateIndex + 1).some(
    (event) => getFunctionResponses(event).some(
...
function mapPlainTextConfirmation(events) {
  ...
  for (let i = turnIndex - 1; i >= 0; i--) {
    ...
    for (const fc of getFunctionCalls(event)) {
      if (fc.name === REQUEST_CONFIRMATION_FUNCTION_CALL_NAME && fc.id && !answered.has(fc.id)) {
        pendingId = fc.id;
        break;
      }
    }
  ...
  return /* @__PURE__ */ new Map([[pendingId, new ToolConfirmation({ confirmed })]]);
}
