...
const _LiveReconnectSignal = class _LiveReconnectSignal extends Error {
  constructor(reason) {
    super("live reconnect requested: ".concat(reason));
    this.reason = reason;
...
const _LlmAgent = class _LlmAgent extends (_b = BaseAgent, _a = LLM_AGENT_SIGNATURE_SYMBOL, _b) {
  ...
  maybeSaveOutputToState(event) {
    ...
    if (this.outputSchema) {
      ...
      try {
        ...
        logger.error(
          "Output for agent ".concat(this.name, " does not satisfy its output schema"),
          e
        );
        result = parsed;
      }
   ...
   */
  runImpl(ctx, nodeInput) {
    return __asyncGenerator(this, null, function* () {
      yield* __yieldStar(runLlmAgentAsNode(this, ctx, nodeInput));
    });
  }
  ...
  runLiveFlow(invocationContext) {
    return __asyncGenerator(this, null, function* () {
      ...
      try {
        for (var iter = __forAwait(this.runLivePreprocess(
          invocationContext,
          llmRequest
      ...
      while (true) {
        ...
        let connection;
        try {
          connection = yield new __await(llm.connect(llmRequest));
        } catch (err) {
          if (isRecoverableLiveError(err) && invocationContext.liveSessionResumptionHandle) {
            ...
            if (reconnectAttempts > MAX_LIVE_RECONNECT_ATTEMPTS) {
              logger.error(
                "Max live reconnection attempts reached (".concat(reconnectAttempts, ")."),
                err
              );
              throw err;
            }
            logger.info(
              "Live connect failed (attempt ".concat(reconnectAttempts, "); retrying with session handle."),
              err
            );
            continue;
          }
        ...
        if (llmRequest.contents.length > 0 && !invocationContext.liveSessionResumptionHandle) {
          yield new __await(connection.sendHistory(llmRequest.contents));
        }
        let sendError;
        ...
        sendTask.catch((error2) => {
          logger.error("Error in live send loop:", error2);
          sendError = error2;
          sendAbort.abort(error2);
        ...
        let reconnect = false;
        try {
          yield* __yieldStar(this.runReceiveLoop(
            invocationContext,
            connection,
            llmRequest,
            sendAbort
          ...
          if (canReconnect) {
            ...
            yield new __await(this.teardownLiveConnection(sendAbort, connection, sendTask));
            throw err;
          }
        }
        yield new __await(this.teardownLiveConnection(sendAbort, connection, sendTask));
  ...
  runLivePreprocess(invocationContext, llmRequest) {
    return __asyncGenerator(this, null, function* () {
      var _a2, _b2;
  ...
  runReceiveLoop(invocationContext, connection, llmRequest, sendAbort) {
    return __asyncGenerator(this, null, function* () {
      var _a2, _b2, _c, _d;
      try {
        for (var iter3 = __forAwait(connection.receive()), more3, temp3, error3; more3 = !(temp3 = yield new __await(iter3.next())).done; more3 = false) {
          ...
          try {
            for (var iter2 = __forAwait(this.postprocessLive(
              invocationContext,
              llmRequest,
              llmResponse,
              modelResponseEvent
              ...
              if (event.content && getFunctionResponses(event).length > 0) {
                yield new __await(connection.sendContent(event.content));
              }
              const taskCompleted = getFunctionResponses(event).some(
                (r) => r.name === "task_completed"
              );
              if (taskCompleted) {
                yield new __await(sleep(TRANSFER_AGENT_DELAY_MS));
                return;
              }
              ...
              if (transferTo) {
                yield new __await(sleep(TRANSFER_AGENT_DELAY_MS));
                sendAbort.abort();
                yield new __await(connection.close());
                ...
                if (subAgent) {
                  ...
                  try {
                    try {
                      for (var iter = __forAwait(subAgent.runLive(
                        invocationContext
                      )), more, temp, error; more = !(temp = yield new __await(iter.next())).done; more = false) {
                        const subEvent = temp.value;
                        yield subEvent;
                      }
                    } catch (temp) {
                      error = [temp];
  ...
  postprocessLive(invocationContext, llmRequest, llmResponse, modelResponseEvent) {
    return __asyncGenerator(this, null, function* () {
      ...
      if (llmResponse.liveSessionResumptionUpdate) {
        yield createEvent(__spreadProps(__spreadValues({}, modelResponseEvent), {
  ...
  runOneStepAsync(invocationContext) {
    return __asyncGenerator(this, null, function* () {
      ...
      yield* __yieldStar(runAsyncGeneratorWithOtelContext(
        ...
        function() {
          return __asyncGenerator(this, null, function* () {
            const responsesGenerator = /* @__PURE__ */ __name(function() {
              return __asyncGenerator(this, null, function* () {
                var _a3, _b3;
                try {
    ...
    return __asyncGenerator(this, null, function* () {
      ...
      const toolTask = (async () => {
        try {
          ...
          return { event: null, error: error4 };
        } finally {
          eventQueue.close();
        }
      })();
      try {
        for (var iter2 = __forAwait(eventQueue), more2, temp2, error2; more2 = !(temp2 = yield new __await(iter2.next())).done; more2 = false) {
          const queuedEvent = temp2.value;
          yield queuedEvent;
        }
      } catch (temp2) {
        error2 = [temp2];
      ...
      const { event: functionResponseEvent, error: toolError } = yield new __await(toolTask);
      ...
      if (nextAgentName) {
        ...
        try {
          for (var iter3 = __forAwait(nextAgent.runAsync(invocationContext)), more3, temp3, error3; more3 = !(temp3 = yield new __await(iter3.next())).done; more3 = false) {
            const event = temp3.value;
