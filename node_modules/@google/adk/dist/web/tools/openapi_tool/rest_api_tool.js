...
let RestApiTool = class extends BaseTool {
  ...
  async runAsync(request) {
    ...
    try {
      ...
      return {
        error: "Failed to execute API call: ".concat(error.message)
      };
    }
  }
};
...
function encodePathParamValue(name, value) {
  if (value === "." || value === "..") {
    throw new Error(
      "Invalid value for path parameter '".concat(name, "': relative path segments ") + "('.' and '..') are not allowed."
    );
  }
...
function prepareRequestParams(endpoint, parameters, args) {
  ...
  let url = "".concat(endpoint.baseUrl).concat(resolvedPath);
