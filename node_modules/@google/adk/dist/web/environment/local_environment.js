...
function resolvePathInWorkingDir(workingDir, filePath) {
  ...
  if (relative === ".." || relative.startsWith("..".concat(path.sep)) || // `path.relative` returns an absolute path across Windows drives.
  path.isAbsolute(relative)) {
    throw new Error("Path escapes working directory: ".concat(filePath));
  }
  return resolved;
}
...
let LocalEnvironment = class extends BaseEnvironment {
  ...
  async initialize() {
    if (this.currentWorkingDir === void 0) {
      ...
      logger.debug("Created temporary workspace: ".concat(this.currentWorkingDir));
    } else {
  ...
  async close() {
    if (this.autoCreated && this.currentWorkingDir !== void 0) {
      ...
      logger.debug("Removed temporary workspace: ".concat(this.currentWorkingDir));
  ...
  async execute(command, timeoutSeconds) {
    ...
    const child = spawn(command, {
      shell: true,
      cwd: this.workingDir,
      env: __spreadValues(__spreadValues({}, process.env), this.envVars)
    });
    const stdoutChunks = [];
    const stderrChunks = [];
