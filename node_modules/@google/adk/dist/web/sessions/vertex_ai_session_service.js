...
__name(isVertexAiConnectionString, "isVertexAiConnectionString");
function quoteFilterLiteral(value) {
  const escaped = value.replace(/\\/g, "\\\\").replace(/"/g, '\\"');
  return '"'.concat(escaped, '"');
}
__name(quoteFilterLiteral, "quoteFilterLiteral");
let VertexAiSessionService = class extends BaseSessionService {
  constructor(options) {
    ...
    if (options.sessions) {
      ...
      if (!this.projectId || !this.location) {
        throw new Error(
          this.expressModeApiKey ? EXPRESS_MODE_UNSUPPORTED_MESSAGE : "Project ID and Location are required."
        );
      }
  ...
  async createSession({
    ...
    let apiResponse = await this.sessions.createInternal({
      ...
      userId,
      config: __spreadValues(__spreadValues(__spreadValues(__spreadValues({}, filteredState ? { sessionState: filteredState } : {}), sessionId ? { sessionId } : {}), ttl != null ? { ttl } : {}), expireTime != null ? { expireTime } : {})
    });
  ...
  async getSession({
    ...
    try {
      ...
      const err = error;
      if (err.code === GRPC_NOT_FOUND || err.code === HTTP_NOT_FOUND || err.status === HTTP_NOT_FOUND) {
        return void 0;
      }
  ...
  async appendEvent({
    ...
    config.rawEvent = JSON.parse(JSON.stringify(__spreadProps(__spreadValues({}, event), { content })));
    const params = {
    ...
    try {
      ...
      logger.warn(
        "Failed to append event with rawEvent; retrying without it. The event will be reconstructed from its structured fields and customMetadata on read.",
        error
      );
...
__name(VertexAiSessionService, "VertexAiSessionService");
VertexAiSessionService = __decorateClass([
  experimental
], VertexAiSessionService);
function toWorkflowMetadata(event) {
  var _a, _b;
  const metadata = {};
  ...
  if (event.nodeInfo !== void 0) {
    metadata.nodeInfo = event.nodeInfo;
  }
  if (event.isolationScope !== void 0) {
    metadata.isolationScope = event.isolationScope;
  }
  if (((_a = event.actions) == null ? void 0 : _a.agentState) !== void 0) {
    metadata.agentState = event.actions.agentState;
  }
  if (((_b = event.actions) == null ? void 0 : _b.endOfAgent) !== void 0) {
    metadata.endOfAgent = event.actions.endOfAgent;
  }
  return Object.keys(metadata).length > 0 ? metadata : void 0;
}
__name(toWorkflowMetadata, "toWorkflowMetadata");
function applyWorkflowMetadata(event, metadata) {
  if (metadata.output !== void 0) {
    event.output = metadata.output;
  }
  if (metadata.route !== void 0) {
    event.route = metadata.route;
  }
  if (metadata.nodeInfo !== void 0) {
    event.nodeInfo = metadata.nodeInfo;
  }
  if (metadata.isolationScope !== void 0) {
    event.isolationScope = metadata.isolationScope;
  }
  if (metadata.agentState !== void 0) {
    event.actions.agentState = metadata.agentState;
  }
  if (metadata.endOfAgent !== void 0) {
    event.actions.endOfAgent = metadata.endOfAgent;
  }
}
__name(applyWorkflowMetadata, "applyWorkflowMetadata");
function toApiActions(actions) {
  if (!actions) {
    return void 0;
  }
  const _a = actions, { transferToAgent } = _a, rest = __objRest(_a, ["transferToAgent"]);
  return __spreadValues(__spreadValues({}, rest), transferToAgent !== void 0 ? { transferAgent: transferToAgent } : {});
}
__name(toApiActions, "toApiActions");
function dropUnsupportedPartFields(content) {
  if (!(content == null ? void 0 : content.parts)) {
    return content;
  }
  return __spreadProps(__spreadValues({}, content), {
    parts: content.parts.map((part) => {
      const copy = __spreadValues({}, part);
      delete copy.partMetadata;
      return copy;
    })
  });
}
__name(dropUnsupportedPartFields, "dropUnsupportedPartFields");
function isInvalidArgumentError(error) {
  return (error == null ? void 0 : error.status) === HTTP_BAD_REQUEST;
}
...
function _fromApiEvent(apiEventObj) {
  ...
  let workflowData;
  if (customMetadata) {
    customMetadata = __spreadValues({}, customMetadata);
    ...
    if (customMetadata[WORKFLOW_CUSTOM_METADATA_KEY]) {
      workflowData = customMetadata[WORKFLOW_CUSTOM_METADATA_KEY];
  ...
  const eventActions = {
    ...
    transferToAgent: (_b = actions["transferAgent"]) != null ? _b : actions["transferToAgent"],
    escalate: actions["escalate"],
    compaction: compactionData || void 0
  };
  const event = {
    id: ((_c = apiEventObj.name) == null ? void 0 : _c.split("/").pop()) || "",
    ...
    errorCode: (_d = apiEventObj.errorCode) == null ? void 0 : _d.toString(),
...
__name(_fromApiEvent, "_fromApiEvent");
export {
  VertexAiSessionService,
