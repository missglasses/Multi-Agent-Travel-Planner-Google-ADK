  ...
  for (const routeKey of keys) {
    ...
    if (Array.isArray(target)) {
      for (const node of target) {
        if (!isNodeLike(node)) {
          throw new Error(
            "Invalid node in fan-out tuple for route ".concat(String(routeKey), ".")
          );
        }
      }
    } else if (!isNodeLike(target)) {
      throw new Error(
        "Invalid routing map value for route ".concat(String(routeKey), ".")
      );
    }
    expanded.push([
      fromElement,
...
function parseEdgeItems(edgeItems) {
  ...
  for (const item of edgeItems) {
    if (isEdge(item)) {
      ...
      throw new Error("Invalid edge item type: ".concat(typeof item));
    }
  }
  return out;
}
