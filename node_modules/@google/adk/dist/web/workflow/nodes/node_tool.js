...
const _NodeTool = class _NodeTool extends (_b = BaseTool, _a = NODE_TOOL_SIGNATURE_SYMBOL, _b) {
  constructor(node, name, description) {
    if (!node.inputSchema) {
      throw new Error(
        "Node '".concat(node.name, "' does not have an inputSchema defined. NodeTool ") + "requires an explicit input schema on the wrapped node."
      );
    }
    super({
      name: name != null ? name : node.name,
      description: description || node.description || "Executes the node: ".concat(node.name),
      isLongRunning: true
    });
  ...
  async runNode(toolContext, nodeInput) {
    ...
    if (!channel) {
      throw new Error(
        "NodeTool '".concat(this.name, "' requires an invocation event queue; ") + "it must be invoked from an LlmAgent tool-call step."
      );
    }
    const runId = toolContext.functionCallId;
    if (!runId) {
      throw new Error(
        "NodeTool '".concat(this.name, "' requires a function-call id; ") + "it must be invoked from an LlmAgent tool-call step."
      );
    }
    if (ic.nodeToolDepth >= MAX_NODE_TOOL_DEPTH) {
      throw new Error(
        "NodeTool '".concat(this.name, "': node-tool nesting exceeded ") + "".concat(MAX_NODE_TOOL_DEPTH, " (possible node -> tool -> node recursion).")
      );
    }
    ...
    const segment = "".concat(this.name, "@").concat(runId);
    const overrideBranch = base ? "".concat(base, ".").concat(segment) : segment;
