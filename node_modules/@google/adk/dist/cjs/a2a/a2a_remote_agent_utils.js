...
__name(getUserFunctionCallAt, "getUserFunctionCallAt");
...
__name(getFunctionResponseCallId, "getFunctionResponseCallId");
function toMissingRemoteSessionParts(ctx, session) {
  ...
  for (let i = events.length - 1; i >= 0; i--) {
    ...
    if (event.author === (0, import_invocation_context.requireAgent)(ctx).name) {
  ...
  for (let i = lastRemoteResponseIndex + 1; i < events.length; i++) {
    let event = events[i];
    if (event.author !== "user" && event.author !== (0, import_invocation_context.requireAgent)(ctx).name) {
...
__name(toMissingRemoteSessionParts, "toMissingRemoteSessionParts");
...
__name(presentAsUserMessage, "presentAsUserMessage");
