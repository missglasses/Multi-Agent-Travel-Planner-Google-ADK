/**
  * @license
  * Copyright 2026 Google LLC
  * SPDX-License-Identifier: Apache-2.0
  */

"use strict";
var __defProp = Object.defineProperty;
var __getOwnPropDesc = Object.getOwnPropertyDescriptor;
var __getOwnPropNames = Object.getOwnPropertyNames;
var __hasOwnProp = Object.prototype.hasOwnProperty;
var __name = (target, value) => __defProp(target, "name", { value, configurable: true });
var __export = (target, all) => {
  for (var name in all)
    __defProp(target, name, { get: all[name], enumerable: true });
};
var __copyProps = (to, from, except, desc) => {
  if (from && typeof from === "object" || typeof from === "function") {
    for (let key of __getOwnPropNames(from))
      if (!__hasOwnProp.call(to, key) && key !== except)
        __defProp(to, key, { get: () => from[key], enumerable: !(desc = __getOwnPropDesc(from, key)) || desc.enumerable });
  }
  return to;
};
var __toCommonJS = (mod) => __copyProps(__defProp({}, "__esModule", { value: true }), mod);
var content_processor_utils_exports = {};
__export(content_processor_utils_exports, {
  getContents: () => getContents,
  getCurrentTurnContents: () => getCurrentTurnContents,
  mergeFunctionResponseEvents: () => mergeFunctionResponseEvents,
  removeClientFunctionCallId: () => removeClientFunctionCallId
});
module.exports = __toCommonJS(content_processor_utils_exports);
var import_genai = require("@google/genai");
var import_lodash_es = require("lodash-es");
var import_compacted_event = require("../../events/compacted_event.js");
var import_event = require("../../events/event.js");
var import_branch_trie = require("../../utils/branch_trie.js");
var import_functions = require("../functions.js");
/**
 * @license
 * Copyright 2025 Google LLC
 * SPDX-License-Identifier: Apache-2.0
 */
function removeClientFunctionCallId(content) {
  var _a, _b, _c, _d, _e;
  for (const part of (_a = content == null ? void 0 : content.parts) != null ? _a : []) {
    if ((_c = (_b = part.functionCall) == null ? void 0 : _b.id) == null ? void 0 : _c.startsWith(import_functions.AF_FUNCTION_CALL_ID_PREFIX)) {
      part.functionCall.id = void 0;
    }
    if ((_e = (_d = part.functionResponse) == null ? void 0 : _d.id) == null ? void 0 : _e.startsWith(import_functions.AF_FUNCTION_CALL_ID_PREFIX)) {
      part.functionResponse.id = void 0;
    }
  }
}
__name(removeClientFunctionCallId, "removeClientFunctionCallId");
function getContents(events, agentName, currentBranch, currentIsolationScope) {
  const filteredEvents = [];
  for (const event of events) {
    if ((0, import_compacted_event.isCompactedEvent)(event)) {
      filteredEvents.push(convertCompactedEvent(event));
      continue;
    }
    if (!shouldIncludeEventInContext(event, currentBranch, currentIsolationScope)) {
      continue;
    }
    filteredEvents.push(
      isEventFromAnotherAgent(agentName, event) ? convertForeignEvent(event) : event
    );
  }
  let resultEvents = rearrangeEventsForLatestFunctionResponse(filteredEvents);
  resultEvents = rearrangeEventsForAsyncFunctionResponsesInHistory(resultEvents);
  const contents = [];
  for (const event of resultEvents) {
    const content = (0, import_lodash_es.cloneDeep)(event.content);
    removeClientFunctionCallId(content);
    contents.push(content);
  }
  return contents;
}
__name(getContents, "getContents");
function shouldIncludeEventInContext(event, currentBranch, currentIsolationScope) {
  var _a, _b, _c;
  if (!((_a = event.content) == null ? void 0 : _a.role) || ((_c = (_b = event.content.parts) == null ? void 0 : _b[0]) == null ? void 0 : _c.text) === "") {
    return false;
  }
  if (currentBranch && event.branch && !(0, import_branch_trie.isSegmentPrefix)(currentBranch, event.branch)) {
    return false;
  }
  if (isOutsideIsolationScope(event, currentIsolationScope)) {
    return false;
  }
  return !isAuthEvent(event) && !isToolConfirmationEvent(event) && !isRequestInputEvent(event);
}
__name(shouldIncludeEventInContext, "shouldIncludeEventInContext");
function isRequestInputEvent(event) {
  var _a, _b, _c;
  if (!((_a = event.content) == null ? void 0 : _a.parts)) {
    return false;
  }
  for (const part of event.content.parts) {
    if (((_b = part.functionCall) == null ? void 0 : _b.name) === import_functions.REQUEST_INPUT_FUNCTION_CALL_NAME || ((_c = part.functionResponse) == null ? void 0 : _c.name) === import_functions.REQUEST_INPUT_FUNCTION_CALL_NAME) {
      return true;
    }
  }
  return false;
}
__name(isRequestInputEvent, "isRequestInputEvent");
function getCurrentTurnContents(events, agentName, currentBranch, currentIsolationScope) {
  for (let i = events.length - 1; i >= 0; i--) {
    const event = events[i];
    if (!shouldIncludeEventInContext(event, currentBranch, currentIsolationScope)) {
      continue;
    }
    if (event.author === "user" || isEventFromAnotherAgent(agentName, event)) {
      return getContents(
        events.slice(turnStart(events, i)),
        agentName,
        currentBranch,
        currentIsolationScope
      );
    }
  }
  return [];
}
__name(getCurrentTurnContents, "getCurrentTurnContents");
function turnStart(events, anchor) {
  var _a, _b, _c, _d, _e;
  const answered = /* @__PURE__ */ new Set();
  for (const event of events.slice(anchor)) {
    for (const part of (_b = (_a = event.content) == null ? void 0 : _a.parts) != null ? _b : []) {
      if ((_c = part.functionResponse) == null ? void 0 : _c.id) {
        answered.add(part.functionResponse.id);
      }
    }
  }
  if (answered.size === 0) {
    return anchor;
  }
  let start = anchor;
  for (let i = anchor - 1; i >= 0; i--) {
    const answersACall = ((_e = (_d = events[i].content) == null ? void 0 : _d.parts) != null ? _e : []).some(
      (p) => {
        var _a2;
        return ((_a2 = p.functionCall) == null ? void 0 : _a2.id) ? answered.has(p.functionCall.id) : false;
      }
    );
    if (answersACall) {
      start = i;
    }
  }
  return start;
}
__name(turnStart, "turnStart");
function isOutsideIsolationScope(event, currentIsolationScope) {
  return event.isolationScope !== void 0 && event.isolationScope !== currentIsolationScope;
}
__name(isOutsideIsolationScope, "isOutsideIsolationScope");
function isAuthEvent(event) {
  var _a, _b, _c;
  if (!((_a = event.content) == null ? void 0 : _a.parts)) {
    return false;
  }
  for (const part of event.content.parts) {
    if (((_b = part.functionCall) == null ? void 0 : _b.name) === import_functions.REQUEST_CREDENTIAL_FUNCTION_CALL_NAME || ((_c = part.functionResponse) == null ? void 0 : _c.name) === import_functions.REQUEST_CREDENTIAL_FUNCTION_CALL_NAME) {
      return true;
    }
  }
  return false;
}
__name(isAuthEvent, "isAuthEvent");
function isToolConfirmationEvent(event) {
  var _a, _b, _c;
  if (!((_a = event.content) == null ? void 0 : _a.parts)) {
    return false;
  }
  for (const part of event.content.parts) {
    if (((_b = part.functionCall) == null ? void 0 : _b.name) === import_functions.REQUEST_CONFIRMATION_FUNCTION_CALL_NAME || ((_c = part.functionResponse) == null ? void 0 : _c.name) === import_functions.REQUEST_CONFIRMATION_FUNCTION_CALL_NAME) {
      return true;
    }
  }
  return false;
}
__name(isToolConfirmationEvent, "isToolConfirmationEvent");
function isEventFromAnotherAgent(agentName, event) {
  return !!agentName && event.author !== agentName && event.author !== "user";
}
__name(isEventFromAnotherAgent, "isEventFromAnotherAgent");
function convertForeignEvent(event) {
  var _a, _b, _c, _d, _e, _f;
  if (!((_b = (_a = event.content) == null ? void 0 : _a.parts) == null ? void 0 : _b.length)) {
    return event;
  }
  const content = {
    role: "user",
    parts: [
      {
        text: "For context:"
      }
    ]
  };
  for (const part of event.content.parts) {
    if (part.text && !part.thought) {
      (_c = content.parts) == null ? void 0 : _c.push({
        text: `[${event.author}] said: ${part.text}`
      });
    } else if (part.functionCall) {
      (_d = content.parts) == null ? void 0 : _d.push({
        text: `[${event.author}] called tool \`${part.functionCall.name}\` with parameters: ${safeStringify(
          part.functionCall.args
        )}`
      });
    } else if (part.functionResponse) {
      (_e = content.parts) == null ? void 0 : _e.push({
        text: `[${event.author}] tool \`${part.functionResponse.name}\` returned result: ${safeStringify(
          part.functionResponse.response
        )}`
      });
    } else {
      (_f = content.parts) == null ? void 0 : _f.push((0, import_lodash_es.cloneDeep)(part));
    }
  }
  return (0, import_event.createEvent)({
    invocationId: event.invocationId,
    author: "user",
    content,
    branch: event.branch,
    timestamp: event.timestamp
  });
}
__name(convertForeignEvent, "convertForeignEvent");
function mergeFunctionResponseEvents(events) {
  var _a;
  if (events.length === 0) {
    throw new Error("Cannot merge an empty list of events.");
  }
  const mergedEvent = (0, import_event.createEvent)({
    ...events[0],
    content: events[0].content ? (0, import_lodash_es.cloneDeep)(events[0].content) : void 0
  });
  const partsInMergedEvent = (_a = mergedEvent.content) == null ? void 0 : _a.parts;
  if (!partsInMergedEvent || partsInMergedEvent.length === 0) {
    throw new Error("There should be at least one function_response part.");
  }
  const partIndicesInMergedEvent = {};
  for (let i = 0; i < partsInMergedEvent.length; i++) {
    const part = partsInMergedEvent[i];
    if (part.functionResponse && part.functionResponse.id) {
      partIndicesInMergedEvent[part.functionResponse.id] = i;
    }
  }
  for (const event of events.slice(1)) {
    if (!event.content || !event.content.parts) {
      throw new Error("There should be at least one function_response part.");
    }
    for (const part of event.content.parts) {
      const clonedPart = (0, import_lodash_es.cloneDeep)(part);
      if (clonedPart.functionResponse && clonedPart.functionResponse.id) {
        const functionCallId = clonedPart.functionResponse.id;
        if (functionCallId in partIndicesInMergedEvent) {
          partsInMergedEvent[partIndicesInMergedEvent[functionCallId]] = clonedPart;
        } else {
          partsInMergedEvent.push(clonedPart);
          partIndicesInMergedEvent[functionCallId] = partsInMergedEvent.length - 1;
        }
      } else {
        partsInMergedEvent.push(clonedPart);
      }
    }
  }
  return mergedEvent;
}
__name(mergeFunctionResponseEvents, "mergeFunctionResponseEvents");
function rearrangeEventsForLatestFunctionResponse(events) {
  if (events.length === 0) {
    return events;
  }
  const latestEvent = events[events.length - 1];
  const functionResponses = (0, import_event.getFunctionResponses)(latestEvent);
  if (!(functionResponses == null ? void 0 : functionResponses.length)) {
    return events;
  }
  const functionResponsesIds = new Set(
    functionResponses.filter((response) => !!response.id).map((response) => response.id)
  );
  const secondLatestEvent = events.at(-2);
  if (secondLatestEvent) {
    const functionCallsFromSecondLatest = (0, import_event.getFunctionCalls)(secondLatestEvent);
    if (functionCallsFromSecondLatest) {
      for (const functionCall of functionCallsFromSecondLatest) {
        if (functionCall.id && functionResponsesIds.has(functionCall.id)) {
          return events;
        }
      }
    }
  }
  let match;
  for (let idx = events.length - 2; idx >= 0; idx--) {
    const event = events[idx];
    const functionCalls = (0, import_event.getFunctionCalls)(event);
    if (!(functionCalls == null ? void 0 : functionCalls.length)) {
      continue;
    }
    let matchedInEvent = false;
    for (const functionCall of functionCalls) {
      if (functionCall.id && functionResponsesIds.has(functionCall.id)) {
        const functionCallIds = new Set(
          functionCalls.map((fc) => fc.id).filter((id) => !!id)
        );
        const isSubset = Array.from(functionResponsesIds).every(
          (id) => functionCallIds.has(id)
        );
        if (!isSubset) {
          throw new Error(
            `Last response event should only contain the responses for the function calls in the same function call event. Function call ids found : ${Array.from(functionCallIds).join(
              ", "
            )}, function response ids provided: ${Array.from(functionResponsesIds).join(", ")}`
          );
        }
        match = { eventIdx: idx, responseIds: functionCallIds };
        matchedInEvent = true;
        break;
      }
    }
    if (matchedInEvent) {
      break;
    }
  }
  if (!match) {
    throw new Error(
      `No function call event found for function responses ids: ${Array.from(
        functionResponsesIds
      ).join(", ")}`
    );
  }
  const functionResponseEvents = [];
  const activeResponses = match.responseIds;
  for (let idx = match.eventIdx + 1; idx < events.length - 1; idx++) {
    const event = events[idx];
    const responses = (0, import_event.getFunctionResponses)(event);
    if (responses && responses.some(
      (response) => response.id && activeResponses.has(response.id)
    )) {
      functionResponseEvents.push(event);
    }
  }
  functionResponseEvents.push(events[events.length - 1]);
  const resultEvents = events.slice(0, match.eventIdx + 1);
  resultEvents.push(mergeFunctionResponseEvents(functionResponseEvents));
  return resultEvents;
}
__name(rearrangeEventsForLatestFunctionResponse, "rearrangeEventsForLatestFunctionResponse");
function rearrangeEventsForAsyncFunctionResponsesInHistory(events) {
  const functionCallIdToResponseEventIndex = /* @__PURE__ */ new Map();
  for (let i = 0; i < events.length; i++) {
    const event = events[i];
    const functionResponses = (0, import_event.getFunctionResponses)(event);
    if (functionResponses == null ? void 0 : functionResponses.length) {
      for (const functionResponse of functionResponses) {
        if (!functionResponse.id) {
          continue;
        }
        functionCallIdToResponseEventIndex.set(functionResponse.id, i);
      }
    }
  }
  const resultEvents = [];
  for (const event of events) {
    if ((0, import_event.getFunctionResponses)(event).length > 0) {
      continue;
    }
    const functionCalls = (0, import_event.getFunctionCalls)(event);
    if (functionCalls == null ? void 0 : functionCalls.length) {
      const functionResponseEventsIndices = /* @__PURE__ */ new Set();
      for (const functionCall of functionCalls) {
        const functionCallId = functionCall.id;
        if (functionCallId && functionCallIdToResponseEventIndex.has(functionCallId)) {
          functionResponseEventsIndices.add(
            functionCallIdToResponseEventIndex.get(functionCallId)
          );
        }
      }
      resultEvents.push(event);
      if (functionResponseEventsIndices.size === 0) {
        continue;
      }
      if (functionResponseEventsIndices.size === 1) {
        const [responseIndex] = Array.from(functionResponseEventsIndices);
        resultEvents.push(events[responseIndex]);
      } else {
        const indicesArray = Array.from(functionResponseEventsIndices).sort(
          (a, b) => a - b
        );
        const eventsToMerge = indicesArray.map((index) => events[index]);
        resultEvents.push(mergeFunctionResponseEvents(eventsToMerge));
      }
    } else {
      resultEvents.push(event);
    }
  }
  return resultEvents;
}
__name(rearrangeEventsForAsyncFunctionResponsesInHistory, "rearrangeEventsForAsyncFunctionResponsesInHistory");
function safeStringify(obj) {
  if (typeof obj === "string") {
    return obj;
  }
  try {
    return JSON.stringify(obj);
  } catch (_e) {
    return String(obj);
  }
}
__name(safeStringify, "safeStringify");
function convertCompactedEvent(event) {
  const content = (0, import_genai.createUserContent)(
    `[Previous Context Summary]:
${event.compactedContent}`
  );
  return (0, import_event.createEvent)({
    invocationId: event.invocationId,
    author: "user",
    content,
    branch: event.branch,
    timestamp: event.timestamp
  });
}
__name(convertCompactedEvent, "convertCompactedEvent");
// Annotate the CommonJS export names for ESM import in node:
0 && (module.exports = {
  getContents,
  getCurrentTurnContents,
  mergeFunctionResponseEvents,
  removeClientFunctionCallId
});
