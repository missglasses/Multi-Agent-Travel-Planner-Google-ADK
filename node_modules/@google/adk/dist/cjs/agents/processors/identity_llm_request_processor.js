/**
  * @license
  * Copyright 2026 Google LLC
  * SPDX-License-Identifier: Apache-2.0
  */

"use strict";
var __defProp = Object.defineProperty;
var __getOwnPropDesc = Object.getOwnPropertyDescriptor;
var __getOwnPropNames = Object.getOwnPropertyNames;
var __hasOwnProp = Object.prototype.hasOwnProperty;
var __name = (target, value) => __defProp(target, "name", { value, configurable: true });
var __export = (target, all) => {
  for (var name in all)
    __defProp(target, name, { get: all[name], enumerable: true });
};
var __copyProps = (to, from, except, desc) => {
  if (from && typeof from === "object" || typeof from === "function") {
    for (let key of __getOwnPropNames(from))
      if (!__hasOwnProp.call(to, key) && key !== except)
        __defProp(to, key, { get: () => from[key], enumerable: !(desc = __getOwnPropDesc(from, key)) || desc.enumerable });
  }
  return to;
};
var __toCommonJS = (mod) => __copyProps(__defProp({}, "__esModule", { value: true }), mod);
var identity_llm_request_processor_exports = {};
__export(identity_llm_request_processor_exports, {
  IDENTITY_LLM_REQUEST_PROCESSOR: () => IDENTITY_LLM_REQUEST_PROCESSOR,
  IdentityLlmRequestProcessor: () => IdentityLlmRequestProcessor
});
module.exports = __toCommonJS(identity_llm_request_processor_exports);
var import_llm_request = require("../../models/llm_request.js");
var import_invocation_context = require("../invocation_context.js");
var import_llm_agent = require("../llm_agent.js");
var import_base_llm_processor = require("./base_llm_processor.js");
/**
 * @license
 * Copyright 2025 Google LLC
 * SPDX-License-Identifier: Apache-2.0
 */
const _IdentityLlmRequestProcessor = class _IdentityLlmRequestProcessor extends import_base_llm_processor.BaseLlmRequestProcessor {
  /**
   * Appends agent name and description as identity instructions to the system
   * prompt of the request, unless the agent has no reachable transfer target.
   *
   * @param invocationContext - The current invocation context.
   * @param llmRequest - The request object to append instructions to.
   */
  // eslint-disable-next-line require-yield
  async *runAsync(invocationContext, llmRequest) {
    const agent = (0, import_invocation_context.requireAgent)(invocationContext);
    if ((0, import_llm_agent.isLlmAgent)(agent) && agent.disallowTransferToParent && agent.disallowTransferToPeers && agent.subAgents.length === 0) {
      return;
    }
    const si = [`You are an agent. Your internal name is "${agent.name}".`];
    if (agent.description) {
      si.push(`The description about you is "${agent.description}"`);
    }
    (0, import_llm_request.appendInstructions)(llmRequest, si);
  }
};
__name(_IdentityLlmRequestProcessor, "IdentityLlmRequestProcessor");
let IdentityLlmRequestProcessor = _IdentityLlmRequestProcessor;
const IDENTITY_LLM_REQUEST_PROCESSOR = new IdentityLlmRequestProcessor();
// Annotate the CommonJS export names for ESM import in node:
0 && (module.exports = {
  IDENTITY_LLM_REQUEST_PROCESSOR,
  IdentityLlmRequestProcessor
});
