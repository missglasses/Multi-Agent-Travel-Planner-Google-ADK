...
var import_finish_task_tool = require("../tools/finish_task_tool.js");
...
var import_run_llm_agent_as_node = require("../workflow/run_llm_agent_as_node.js");
...
var import_output_schema_utils = require("../utils/output_schema_utils.js");
...
var import_request_input_llm_request_processor = require("./processors/request_input_llm_request_processor.js");
...
async function convertToolUnionToTools(toolUnion, context2) {
  ...
  if ((0, import_base_node.isBaseNode)(toolUnion)) {
    return [new import_node_tool.NodeTool(toolUnion)];
  }
...
const _LlmAgent = class _LlmAgent extends (_b = import_base_agent.BaseAgent, _a = LLM_AGENT_SIGNATURE_SYMBOL, _b) {
  constructor(config) {
    ...
    this.requestProcessors = (_i = config.requestProcessors) != null ? _i : [
      ...
      import_request_input_llm_request_processor.REQUEST_INPUT_LLM_REQUEST_PROCESSOR,
  ...
  get finishTaskTool() {
    if (!this._finishTaskTool) {
      this._finishTaskTool = new import_finish_task_tool.FinishTaskTool(this.outputSchema);
    }
  ...
  validateOutput(value) {
    var _a2;
    return (0, import_schema.parseWithSchema)((_a2 = this.outputSchemaSource) != null ? _a2 : this.outputSchema, value);
  }
  /**
  ...
  async *runImpl(ctx, nodeInput) {
    yield* (0, import_run_llm_agent_as_node.runLlmAgentAsNode)(this, ctx, nodeInput);
  }
  ...
  async *runLiveFlow(invocationContext) {
    ...
    while (true) {
      ...
      sendTask.catch((error) => {
        import_logger.logger.error("Error in live send loop:", error);
        sendError = error;
        sendAbort.abort(error);
        connection.close().catch((err) => {
          import_logger.logger.warn("Error closing connection after send loop failure:", err);
        });
      });
      let reconnect = false;
      try {
  ...
  async runSendLoop(connection, liveRequestQueue, sendAbortSignal, invocationAbortSignal) {
    while (true) {
      ...
      try {
        ...
        if ((sendAbortSignal == null ? void 0 : sendAbortSignal.aborted) || (invocationAbortSignal == null ? void 0 : invocationAbortSignal.aborted)) {
          import_logger.logger.debug("Send failed after teardown:", error);
          return;
        }
        import_logger.logger.error("Error dispatching live request to model:", error);
        throw error;
      }
  ...
  async teardownLiveConnection(sendAbort, connection, sendTask) {
    ...
    try {
      ...
      import_logger.logger.warn("Error closing live connection:", error);
    }
  ...
  async *runReceiveLoop(invocationContext, connection, llmRequest, sendAbort) {
    ...
    for await (const llmResponse of connection.receive()) {
      ...
      if (llmResponse.goAway) {
        import_logger.logger.info("Received goAway from live server; triggering reconnect.");
      ...
      for await (const event of this.postprocessLive(
        ...
        yield event;
        if (event.content && (0, import_event.getFunctionResponses)(event).length > 0) {
          await connection.sendContent(event.content);
        }
        const taskCompleted = (0, import_event.getFunctionResponses)(event).some(
        ...
        if (transferTo) {
          ...
          sendAbort.abort();
          await connection.close();
          const agent = (0, import_invocation_context.requireAgent)(invocationContext);
  ...
  async *postprocessLive(invocationContext, llmRequest, llmResponse, modelResponseEvent) {
    ...
    if (mergedEvent.content && functionCalls.length) {
      (0, import_event.populateClientFunctionCallId)(mergedEvent);
  ...
  async *runOneStepAsync(invocationContext) {
    ...
    } else if (this.outputSchema && allTools.length > 0 && !(0, import_output_schema_utils.canUseOutputSchemaWithTools)(this.canonicalModel.model)) {
  ...
  async *postprocess(invocationContext, llmRequest, llmResponse, modelResponseEvent) {
    ...
    const toolTask = (async () => {
      try {
        const event = await (0, import_functions.handleFunctionCallsAsync)({
          invocationContext,
          functionCallEvent: mergedEvent,
          toolsDict: llmRequest.toolsDict,
          beforeToolCallbacks: this.canonicalBeforeToolCallbacks,
  ...
  getAgentByName(invocationContext, agentName) {
    const rootAgent = (0, import_invocation_context.requireAgent)(invocationContext).rootAgent;
