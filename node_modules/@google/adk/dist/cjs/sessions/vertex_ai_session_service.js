...
let VertexAiSessionService = class extends import_base_session_service.BaseSessionService {
  constructor(options) {
    ...
    if (options.sessions) {
      ...
      if (!this.projectId || !this.location) {
        throw new Error(
          this.expressModeApiKey ? import_vertex_ai_utils.EXPRESS_MODE_UNSUPPORTED_MESSAGE : "Project ID and Location are required."
        );
      }
  ...
  async appendEvent({
    ...
    config.rawEvent = JSON.parse(JSON.stringify({ ...event, content }));
    const params = {
...
function toApiActions(actions) {
  ...
  return {
    ...rest,
    ...transferToAgent !== void 0 ? { transferAgent: transferToAgent } : {}
  };
}
