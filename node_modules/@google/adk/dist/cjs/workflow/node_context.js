...
var node_context_exports = {};
__export(node_context_exports, {
  NodeContext: () => NodeContext
});
module.exports = __toCommonJS(node_context_exports);
...
const _NodeContext = class _NodeContext {
  constructor(opts) {
    ...
    this.actions = (_b = opts.actions) != null ? _b : (0, import_event_actions.createEventActions)();
  ...
  runNode(nodeLike, input, options) {
    var _a, _b;
    const node = (0, import_workflow_graph_utils.buildNode)(nodeLike);
    if (this.scheduler) {
    ...
    return (0, import_node_runner.executeChildNode)({ parent: this, node, input, options });
  }
  /**
  ...
  async scheduleAndUnwind(node, input, options) {
    ...
    if (child.interruptIds.length > 0) {
      throw new import_errors.NodeInterruptedError();
    }
    return child;
  }
};
...
function assertCustomRunId(runId, nodeName, autoRunIds) {
  if (runId.trim() === "") {
    throw new Error(
      `Invalid runId for node '${nodeName}': a custom run id cannot be empty. Omit runId to let ADK number the run automatically.`
    );
  }
