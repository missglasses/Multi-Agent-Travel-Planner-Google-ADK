 ...
 * SPDX-License-Identifier: Apache-2.0
 */
const _AgentRegistrySingleMCPToolset = class _AgentRegistrySingleMCPToolset extends import_base_toolset.BaseToolset {
  /**
  ...
  async getTools(context) {
    ...
    try {
      ...
      await sessionManager.closeSession(session).catch((e) => {
        import_logger.logger.warn("Failed to close MCP discovery session", e);
      });
    }
    const tools = listResult.tools.map((tool) => {
      ...
      if (this.destinationResourceId) {
        ...
        toolWithMetadata.customMetadata[import_types.GCP_MCP_SERVER_DESTINATION_ID] = this.destinationResourceId;
      }
      return mcpTool;
    });
...
__name(_AgentRegistrySingleMCPToolset, "AgentRegistrySingleMCPToolset");
let AgentRegistrySingleMCPToolset = _AgentRegistrySingleMCPToolset;
