  ...
  getNextPendingNodes(nodeName, routesToMatch) {
    ...
    if (nextPending.length === 0 && availableRoutes.length > 0) {
      ...
      if (emitted.length > 0 && !isConditionalContinue) {
        ...
        logger.debug(
          `Node '${nodeName}' emitted route ${shown}, which matches no outgoing edge, so this branch stops here. Edges from this node are keyed to: ${known}.`
        );
      }
    }
    return nextPending;
