...
let Workflow = class extends (_b = BaseNode, _a = WORKFLOW_SIGNATURE_SYMBOL, _b) {
  constructor(config) {
    var _a2;
  ...
  reportNodeError(loop, ctx, nodeName, error) {
    var _a2, _b2, _c;
    if (isInvocationAbortedError(error) || ((_a2 = loop.abortSignal) == null ? void 0 : _a2.aborted)) {
      return;
    }
  ...
  scheduleReadyNodes(loop, ctx) {
    for (const nodeName of [...loop.triggerBuffer.keys()]) {
      ...
      if (state) {
        ...
        if (state.status === NodeStatus.WAITING && state.interrupts.length > 0) {
          continue;
        }
      }
    ...
    for (const targetName of nextNodes) {
      ...
      if (targetNode.requiresAllPredecessors) {
        ...
        const allCompleted = [...predecessors].every(
          (p) => {
            var _a2;
            return ((_a2 = loop.nodes.get(p)) == null ? void 0 : _a2.status) === NodeStatus.COMPLETED;
          }
        );
        if (allCompleted) {
          const outputs = {};
  ...
  collectRemainingInterrupts(loop) {
    for (const nodeState of loop.nodes.values()) {
      if (nodeState.status === NodeStatus.WAITING && nodeState.interrupts.length > 0) {
