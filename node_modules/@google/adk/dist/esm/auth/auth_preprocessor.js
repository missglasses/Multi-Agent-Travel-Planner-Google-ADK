...
function requestedAuthConfigs(events, authFcIds, agentName) {
  ...
  for (const event of events) {
    ...
    for (const functionCall of getFunctionCalls(event)) {
      if (!functionCall.id || !authFcIds.has(functionCall.id) || functionCall.name !== REQUEST_CREDENTIAL_FUNCTION_CALL_NAME) {
        continue;
      }
      const args = camelCaseKeys(functionCall.args);
      if (args == null ? void 0 : args.authConfig) {
...
__name(requestedAuthConfigs, "requestedAuthConfigs");
async function storeAuthAndCollectResumeTargets(events, authFcIds, authResponses, state, agentName) {
  var _a;
  ...
  for (const fcId of authFcIds) {
    ...
    const functionCallId = (_a = request.args) == null ? void 0 : _a.functionCallId;
    if (functionCallId && !functionCallId.startsWith(TOOLSET_AUTH_CREDENTIAL_ID_PREFIX)) {
      toolsToResume.add(functionCallId);
    }
  }
  return toolsToResume;
}
__name(storeAuthAndCollectResumeTargets, "storeAuthAndCollectResumeTargets");
const _AuthPreprocessor = class _AuthPreprocessor extends BaseLlmRequestProcessor {
...
__name(_AuthPreprocessor, "AuthPreprocessor");
let AuthPreprocessor = _AuthPreprocessor;
