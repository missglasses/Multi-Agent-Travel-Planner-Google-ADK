...
function shouldIncludeEventInContext(event, currentBranch, currentIsolationScope) {
  ...
  if (currentBranch && event.branch && !isSegmentPrefix(currentBranch, event.branch)) {
    return false;
  }
...
function isRequestInputEvent(event) {
  ...
  for (const part of event.content.parts) {
    if (((_b = part.functionCall) == null ? void 0 : _b.name) === REQUEST_INPUT_FUNCTION_CALL_NAME || ((_c = part.functionResponse) == null ? void 0 : _c.name) === REQUEST_INPUT_FUNCTION_CALL_NAME) {
      return true;
    }
  }
  return false;
...
function isAuthEvent(event) {
  ...
  for (const part of event.content.parts) {
    if (((_b = part.functionCall) == null ? void 0 : _b.name) === REQUEST_CREDENTIAL_FUNCTION_CALL_NAME || ((_c = part.functionResponse) == null ? void 0 : _c.name) === REQUEST_CREDENTIAL_FUNCTION_CALL_NAME) {
      return true;
    }
  }
  return false;
