import {createRequire as topLevelCreateRequire} from 'module';
const require = topLevelCreateRequire(import.meta.url);
var __defProp = Object.defineProperty;
var __getOwnPropDesc = Object.getOwnPropertyDescriptor;
var __name = (target, value) => __defProp(target, "name", { value, configurable: true });
var __decorateClass = (decorators, target, key, kind) => {
  var result = kind > 1 ? void 0 : kind ? __getOwnPropDesc(target, key) : target;
  for (var i = decorators.length - 1, decorator; i >= 0; i--)
    if (decorator = decorators[i])
      result = (kind ? decorator(target, key, result) : decorator(result)) || result;
  if (kind && result) __defProp(target, key, result);
  return result;
};
/**
 * @license
 * Copyright 2025 Google LLC
 * SPDX-License-Identifier: Apache-2.0
 */
var _a, _b;
import { deprecated } from "../utils/deprecated.js";
import { BaseAgent } from "./base_agent.js";
const LOOP_AGENT_SIGNATURE_SYMBOL = Symbol.for("google.adk.loopAgent");
function isLoopAgent(obj) {
  return typeof obj === "object" && obj !== null && LOOP_AGENT_SIGNATURE_SYMBOL in obj && obj[LOOP_AGENT_SIGNATURE_SYMBOL] === true;
}
__name(isLoopAgent, "isLoopAgent");
let LoopAgent = class extends (_b = BaseAgent, _a = LOOP_AGENT_SIGNATURE_SYMBOL, _b) {
  constructor(config) {
    var _a2;
    super(config);
    /**
     * A unique symbol to identify ADK loop agent class.
     */
    this[_a] = true;
    this.maxIterations = (_a2 = config.maxIterations) != null ? _a2 : Number.MAX_SAFE_INTEGER;
  }
  async *runAsyncImpl(context) {
    var _a2;
    let iteration = 0;
    while (iteration < this.maxIterations) {
      for (const subAgent of this.subAgents) {
        let shouldExit = false;
        for await (const event of subAgent.runAsync(context)) {
          if ((_a2 = context.abortSignal) == null ? void 0 : _a2.aborted) {
            return;
          }
          yield event;
          if (event.actions.escalate) {
            shouldExit = true;
          }
        }
        if (shouldExit) {
          return;
        }
      }
      iteration++;
    }
    return;
  }
  async *runLiveImpl(context) {
    var _a2, _b2;
    for (let iteration = 0; iteration < this.maxIterations; iteration++) {
      for (const subAgent of this.subAgents) {
        for await (const event of subAgent.runLive(context)) {
          if ((_a2 = context.abortSignal) == null ? void 0 : _a2.aborted) {
            return;
          }
          yield event;
          if ((_b2 = event.actions) == null ? void 0 : _b2.escalate) {
            return;
          }
        }
      }
    }
  }
};
__name(LoopAgent, "LoopAgent");
LoopAgent = __decorateClass([
  deprecated(
    "LoopAgent is deprecated in favor of Workflow and will be removed in a future version. Workflow cannot yet be used as an LlmAgent sub-agent."
  )
], LoopAgent);
export {
  LoopAgent,
  isLoopAgent
};
