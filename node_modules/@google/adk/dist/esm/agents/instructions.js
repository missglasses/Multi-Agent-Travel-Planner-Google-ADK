import {createRequire as topLevelCreateRequire} from 'module';
const require = topLevelCreateRequire(import.meta.url);
var __defProp = Object.defineProperty;
var __name = (target, value) => __defProp(target, "name", { value, configurable: true });
/**
 * @license
 * Copyright 2025 Google LLC
 * SPDX-License-Identifier: Apache-2.0
 */
import { State } from "../sessions/state.js";
const ARTIFACT_PREFIX = "artifact.";
const WORKFLOW_FIELD_KEY = /^[A-Za-z_]\w*\.[A-Za-z_]\w*$/;
const SOURCE_NODE_PLACEHOLDER = /<\s*[A-Za-z_]\w*\.([A-Za-z_]\w*)\s+from\s+([A-Za-z_]\w*)\s*>/g;
function resolveSourceNode(raw, field, nodeName, scope) {
  var _a;
  const out = (_a = scope.outputsByNode) == null ? void 0 : _a[nodeName];
  if (out && typeof out === "object" && field in out) {
    return formatValue(out[field], false);
  }
  return raw;
}
__name(resolveSourceNode, "resolveSourceNode");
async function resolveKey(key, isOptional, rawMatch, readonlyContext) {
  const invocationContext = readonlyContext.invocationContext;
  if (key.startsWith(ARTIFACT_PREFIX)) {
    const fileName = key.substring(ARTIFACT_PREFIX.length);
    if (invocationContext.artifactService === void 0) {
      throw new Error("Artifact service is not initialized.");
    }
    const artifact = await invocationContext.artifactService.loadArtifact({
      filename: fileName
    });
    if (!artifact) {
      if (isOptional) {
        return "";
      }
      throw new Error(`Artifact ${fileName} not found.`);
    }
    return formatValue(artifact, true);
  }
  if (isValidStateName(key)) {
    if (key in invocationContext.session.state) {
      return formatValue(invocationContext.session.state[key], false);
    }
    if (isOptional) {
      return "";
    }
    throw new Error(`Context variable not found: \`${key}\`.`);
  }
  const scope = invocationContext.workflowInstructionScope;
  if (scope && WORKFLOW_FIELD_KEY.test(key)) {
    const field = key.slice(key.indexOf(".") + 1);
    const input = scope.input;
    if (input && typeof input === "object" && field in input) {
      return formatValue(input[field], false);
    }
    if (isOptional) {
      return "";
    }
  }
  return rawMatch;
}
__name(resolveKey, "resolveKey");
function formatValue(value, isArtifact = false) {
  if (typeof value === "string") {
    return value;
  }
  if (typeof value === "object" && value !== null) {
    if (isArtifact && "text" in value && typeof value.text === "string") {
      return value.text;
    }
    try {
      const json = JSON.stringify(value);
      if (json !== void 0) {
        return json;
      }
    } catch (e) {
      throw new Error(
        `Failed to serialize value for instruction template: ${e instanceof Error ? e.message : String(e)}`
      );
    }
  }
  return String(value);
}
__name(formatValue, "formatValue");
async function injectSessionState(template, readonlyContext) {
  const scope = readonlyContext.invocationContext.workflowInstructionScope;
  const sourceMatches = scope ? Array.from(template.matchAll(SOURCE_NODE_PLACEHOLDER)).map((match) => ({
    raw: match[0],
    index: match.index,
    resolved: resolveSourceNode(match[0], match[1], match[2], scope)
  })) : [];
  const pattern = /\{+[^{}]*}+/g;
  const matches = Array.from(template.matchAll(pattern));
  if (matches.length === 0 && sourceMatches.length === 0) {
    return template;
  }
  const parsedMatches = matches.map((match) => {
    const raw = match[0];
    let key = raw.replace(/^\{+/, "").replace(/\}+$/, "").trim();
    const isOptional = key.endsWith("?");
    if (isOptional) {
      key = key.slice(0, -1);
    }
    const isValid = key.startsWith(ARTIFACT_PREFIX) || isValidStateName(key) || !!scope && WORKFLOW_FIELD_KEY.test(key);
    return {
      raw,
      key,
      isOptional,
      isValid,
      index: match.index
    };
  });
  const uniqueKeys = /* @__PURE__ */ new Map();
  for (const pm of parsedMatches) {
    if (!pm.isValid) {
      continue;
    }
    const existing = uniqueKeys.get(pm.key);
    if (existing) {
      if (existing.isOptional && !pm.isOptional) {
        existing.isOptional = false;
      }
    } else {
      uniqueKeys.set(pm.key, {
        key: pm.key,
        isOptional: pm.isOptional,
        raw: pm.raw
      });
    }
  }
  const resolutions = /* @__PURE__ */ new Map();
  for (const info of uniqueKeys.values()) {
    resolutions.set(
      info.key,
      resolveKey(info.key, info.isOptional, info.raw, readonlyContext)
    );
  }
  await Promise.all(resolutions.values());
  const allMatches = [...parsedMatches, ...sourceMatches].sort(
    (a, b) => a.index - b.index
  );
  const result = [];
  let lastEnd = 0;
  for (const pm of allMatches) {
    result.push(template.slice(lastEnd, pm.index));
    if ("resolved" in pm) {
      result.push(pm.resolved);
    } else if (pm.isValid) {
      const replacement = await resolutions.get(pm.key);
      result.push(replacement);
    } else {
      result.push(pm.raw);
    }
    lastEnd = pm.index + pm.raw.length;
  }
  result.push(template.slice(lastEnd));
  return result.join("");
}
__name(injectSessionState, "injectSessionState");
const isIdentifierPattern = /* @__PURE__ */ (() => {
  return /^[a-zA-Z_][a-zA-Z0-9_]*$/;
})();
function isIdentifier(s) {
  if (s === "" || s === void 0) {
    return false;
  }
  return isIdentifierPattern.test(s);
}
__name(isIdentifier, "isIdentifier");
const VALID_PREFIXES = [State.APP_PREFIX, State.USER_PREFIX, State.TEMP_PREFIX];
function isValidStateName(variableName) {
  const parts = variableName.split(":");
  if (parts.length === 0 || parts.length > 2) {
    return false;
  }
  if (parts.length === 1) {
    return isIdentifier(variableName);
  }
  if (VALID_PREFIXES.includes(parts[0] + ":")) {
    return isIdentifier(parts[1]);
  }
  return false;
}
__name(isValidStateName, "isValidStateName");
export {
  injectSessionState
};
