...
__name(getFinalTaskStatusUpdate, "getFinalTaskStatusUpdate");
...
__name(scanForInputRequiredEvents, "scanForInputRequiredEvents");
...
__name(getLongRunnningFunctionCallId, "getLongRunnningFunctionCallId");
function getUnansweredRequestEvent(options) {
  var _a, _b, _c;
  const { taskId, contextId, task, sessionEvents, genAIContent } = options;
  const pending = pendingRequestParts(task, sessionEvents);
  const answered = new Set(
    ((_a = genAIContent == null ? void 0 : genAIContent.parts) != null ? _a : []).map((part) => {
      var _a2;
      return (_a2 = part.functionResponse) == null ? void 0 : _a2.id;
    }).filter((id) => !!id)
  );
  ...
  return createInputMissingErrorEvent({
    taskId: (_b = task == null ? void 0 : task.id) != null ? _b : taskId,
    contextId: (_c = task == null ? void 0 : task.contextId) != null ? _c : contextId,
    parts: [
...
__name(getUnansweredRequestEvent, "getUnansweredRequestEvent");
function pendingRequestParts(task, sessionEvents) {
  ...
  if (task && isInputRequiredTaskStatusUpdateEvent(task) && task.status.message) {
    for (const part of toGenAIParts(task.status.message.parts)) {
      if ((_a = part.functionCall) == null ? void 0 : _a.id) {
  ...
  for (const event of sessionEvents) {
    for (const part of (_c = (_b = event.content) == null ? void 0 : _b.parts) != null ? _c : []) {
  ...
  for (const event of sessionEvents) {
    for (const part of (_e = (_d = event.content) == null ? void 0 : _d.parts) != null ? _e : []) {
      if ((_f = part.functionResponse) == null ? void 0 : _f.id) {
...
__name(pendingRequestParts, "pendingRequestParts");
function interruptIdOfPart(event, part) {
  if (!part.functionCall) {
    return void 0;
  }
  const [request] = getUserInputRequests({ ...event, content: { parts: [part] } });
  return request == null ? void 0 : request.interruptId;
}
__name(interruptIdOfPart, "interruptIdOfPart");
export {
