...
async function generateAgentFolder(agentDir, forceYes) {
  ...
  const overwriteFolderResponse = forceYes ? true : await select({
...
async function createAgent(options) {
  ...
  if (!options.model) {
    const model = options.forceYes ? "gemini-2.5-flash" : await select({
  ...
  if (options.language !== "js" && options.language !== "ts") {
    const language = options.forceYes ? "ts" : await select({
  ...
  if (!options.apiKey && !options.project) {
    const backend = options.forceYes ? "googleai" : await select({
    ...
    if (backend === "vertex") {
      ...
      const projectResponse = options.forceYes ? defaultProject : await text({
      ...
      const regionResponse = options.forceYes ? defaultRegion : await text({
      ...
      const apiKeyResponse = options.forceYes ? "" : await text({
