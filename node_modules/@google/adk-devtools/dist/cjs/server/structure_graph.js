...
var structure_graph_exports = {};
__export(structure_graph_exports, {
  renderStructureGraphAsDot: () => renderStructureGraphAsDot
});
module.exports = __toCommonJS(structure_graph_exports);
...
function renderStructureGraphAsDot(level, darkMode = false) {
  ...
  const dot = new import_ts_graphviz.Digraph(level.name, {
  ...
  for (const node of nodes) {
    ...
    if (isConditional) {
      ...
      dot.addNode(
        new import_ts_graphviz.Node(name, { label, tooltip, style: "rounded,filled,dashed" })
      );
    } else {
      dot.addNode(new import_ts_graphviz.Node(name, { label, tooltip, style: "rounded,filled" }));
    }
  }
  ...
  for (const edge of edges) {
    ...
    if (from === START_NODE_NAME && !startDrawn) {
      startDrawn = true;
      dot.addNode(
        new import_ts_graphviz.Node(START_NODE_NAME, {
          label: "START",
          shape: "oval",
          style: "filled",
    ...
    if (edge.isToolEdge) {
      dot.addEdge(
        new import_ts_graphviz.Edge([new import_ts_graphviz.Node(from), new import_ts_graphviz.Node(to)], {
          style: "dashed",
          color: palette.edge
        })
      );
  ...
  if (isWorkflow && terminals.length) {
    dot.addNode(
      new import_ts_graphviz.Node(END_NODE_NAME, {
        label: "END",
        shape: "oval",
        style: "filled",
    ...
    for (const terminal of terminals) {
      dot.addEdge(new import_ts_graphviz.Edge([new import_ts_graphviz.Node(terminal), new import_ts_graphviz.Node(END_NODE_NAME)]));
    }
  }
  return (0, import_ts_graphviz.toDot)(dot);
}
