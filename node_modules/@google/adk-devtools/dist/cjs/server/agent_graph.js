...
__export(agent_graph_exports, {
  ...
  getWorkflowHighlights: () => getWorkflowHighlights
});
...
async function buildGraph(graph, root, highlightsPairs, parentAgent) {
  if ((0, import_adk.isWorkflow)(root)) {
...
function drawWorkflowCluster(container, workflow, path, highlightsPairs) {
  const cluster = new import_ts_graphviz.Subgraph(`cluster_${path}`, {
  ...
  if (!graph) {
    cluster.addNode(
      new import_ts_graphviz.Node(path, {
...
function drawWorkflowNode(cluster, node, path, highlightsPairs) {
  ...
  if (node.name === WORKFLOW_START_NODE_NAME) {
    cluster.addNode(
      new import_ts_graphviz.Node(id, {
        label: "",
        shape: "point",
        width: 0.15,
        style: "filled",
...
function drawWorkflowEdges(cluster, graph, path, highlightsPairs) {
  ...
  for (const { tail, head, routes, highlighted } of merged.values()) {
    ...
    cluster.addEdge(
      new import_ts_graphviz.Edge([new import_ts_graphviz.Node(tail), new import_ts_graphviz.Node(head)], {
        color,
        fontcolor: color,
...
function getRouteLabels(route) {
  ...
  return routes.map(
    (value) => value === import_adk.DEFAULT_ROUTE ? DEFAULT_ROUTE_LABEL : String(value)
  );
}
...
function classifyWorkflowNode(node) {
  if ((0, import_adk.isBaseAgent)(node)) {
    return { icon: "\u{1F916}", shape: "ellipse" };
  }
  if ((0, import_adk.isBaseTool)(getNodeField(node, "tool"))) {
