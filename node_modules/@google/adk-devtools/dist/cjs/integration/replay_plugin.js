
const importMetaUrl = /* @__PURE__ */ require("node:url").pathToFileURL(__filename).toString();
const importMetaFilename = /* @__PURE__ */ __filename;
const importMetaDirname = /* @__PURE__ */ __dirname;
const importMetaResolve = /* @__PURE__ */ require.resolve;
"use strict";
var __defProp = Object.defineProperty;
var __getOwnPropDesc = Object.getOwnPropertyDescriptor;
var __getOwnPropNames = Object.getOwnPropertyNames;
var __hasOwnProp = Object.prototype.hasOwnProperty;
var __export = (target, all) => {
  for (var name in all)
    __defProp(target, name, { get: all[name], enumerable: true });
};
var __copyProps = (to, from, except, desc) => {
  if (from && typeof from === "object" || typeof from === "function") {
    for (let key of __getOwnPropNames(from))
      if (!__hasOwnProp.call(to, key) && key !== except)
        __defProp(to, key, { get: () => from[key], enumerable: !(desc = __getOwnPropDesc(from, key)) || desc.enumerable });
  }
  return to;
};
var __toCommonJS = (mod) => __copyProps(__defProp({}, "__esModule", { value: true }), mod);
var replay_plugin_exports = {};
__export(replay_plugin_exports, {
  ReplayPlugin: () => ReplayPlugin
});
module.exports = __toCommonJS(replay_plugin_exports);
var import_adk = require("@google/adk");
/**
 * @license
 * Copyright 2026 Google LLC
 * SPDX-License-Identifier: Apache-2.0
 */
class ReplayPlugin extends import_adk.BasePlugin {
  constructor(recordings, context) {
    super("replay-plugin");
    this.recordings = recordings;
    this.context = context;
  }
  async beforeModelCallback({
    callbackContext
  }) {
    const agentName = callbackContext.agentName;
    const index = this.recordings.findIndex(
      (r) => {
        var _a;
        return r.userMessageIndex === this.context.userMessageIndex && r.agentName === agentName && ((_a = r.llmRecording) == null ? void 0 : _a.llmResponse) && // replay internal flag to mark event as consumed
        !r._consumed;
      }
    );
    if (index === -1) {
      throw new Error(
        `No LLM recording found for agent ${agentName} at turn ${this.context.userMessageIndex}`
      );
    }
    const rec = this.recordings[index];
    rec._consumed = true;
    return rec.llmRecording.llmResponse;
  }
  async beforeToolCallback(params) {
    var _a;
    const agentName = ((_a = params.toolContext.invocationContext.agent) == null ? void 0 : _a.name) ?? "";
    const toolName = params.tool.name;
    const index = this.recordings.findIndex(
      (r) => {
        var _a2, _b;
        return r.userMessageIndex === this.context.userMessageIndex && r.agentName === agentName && ((_b = (_a2 = r.toolRecording) == null ? void 0 : _a2.toolCall) == null ? void 0 : _b.name) === toolName && !r._consumed;
      }
    );
    if (index === -1) {
      throw new Error(
        `No tool recording found for agent ${agentName}, tool ${toolName} at turn ${this.context.userMessageIndex}`
      );
    }
    const rec = this.recordings[index];
    rec._consumed = true;
    if (toolName === "transfer_to_agent") {
      params.toolContext.actions.transferToAgent = params.toolArgs["agentName"];
    }
    const response = rec.toolRecording.toolResponse.response;
    if (response instanceof Map) {
      return Object.fromEntries(response);
    }
    return response;
  }
}
// Annotate the CommonJS export names for ESM import in node:
0 && (module.exports = {
  ReplayPlugin
});
